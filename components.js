...
function renderFooter(lang) {
    ...
    if (!footer) return;
    const year = (lang === 'fa') ? getCurrentJalaliYear() : new Date().getFullYear();
    footer.innerHTML = `
        <div class="container">
            <p>© ${year} — ${lang === 'en' ? 'Built with passion. All rights reserved.' : 'ساخته شده با اشتیاق. تمامی حقوق محفوظ است.'}</p>
        </div>
    `;
}
